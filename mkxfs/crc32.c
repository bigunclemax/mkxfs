...
static uint32_t crc32(uint32_t pcrc,void *buf, size_t len)
{
    size_t bytes;
    uint32_t crc = pcrc;
...
int crc32_fd(int fd, uint32_t *crc32val) {
	...
	if(fstat(fd, &sinfo) == -1 || !S_ISREG(sinfo.st_mode)) {
		return -1;
	}
...
int crc32_fd(int fd, uint32_t *crc32val) {
    ...
    /* Ugh, this is what cksum does... encodes the file len in the crc */
    while (len) {
        plen = len & 0xff;
        fcrc = crc32(fcrc, &plen, 1);
        len >>=8;
    }
...
int crc32_fn(char* filename, uint32_t *crc32val) {
	...
	retval = crc32_fd(fd, &cv);
	if(crc32val) *crc32val = cv;
